...
function main() {
  ...
  echo "=== Copy TensorFlow Compress files"
  ...
  #   |- MANIFEST.in (needed)
  #   |- setup.py (needed)
  #   |- tensorflow_compress (needed)
  #
  # To build tensorflow compress wheel, we only need setup.py, MANIFEST.in, and
  # python and .so files under tensorflow_compress/tensorflow_compress.
  # So we extract those to ${TMPDIR}.
  cp bazel-bin/pip_pkg.runfiles/"${PKG_NAME}"/setup.py "${TMPDIR}"
  cp bazel-bin/pip_pkg.runfiles/"${PKG_NAME}"/MANIFEST.in "${TMPDIR}"
  cp -R \
    bazel-bin/pip_pkg.runfiles/"${PKG_NAME}"/"${PKG_NAME}" \
    "${TMPDIR}"

  echo "=== Copy TensorFlow Compress root files"
  cp README.md ${TMPDIR}
  cp LICENSE ${TMPDIR}
  ...
  #  ./pip_pkg /tmp/tensorflow_compress_pkg --gpu --release
